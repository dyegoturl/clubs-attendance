...
export default async function AdminClubsPage() {
  ...
  const [clubsRes, classesRes, regionsRes, programsRes, coachAssignmentsRes] = await Promise.all([
    ...
    supabase.from('active_coach_assignments')
      .select('class_id, club_id, class_identifier, class_type, gender, time_start, time_end, days_of_week, coach_name')
      .order('time_start'),
  ])
  ...
  return (
    <AppShell role="admin" userName={profile?.name ?? 'Admin'}>
      <ClubsManager
        ...
        classesWithCoach={coachAssignmentsRes.data ?? []}
      />
    </AppShell>
  )
}
